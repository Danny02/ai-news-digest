...
# put_worker_secret NAME VALUE — set a Worker secret via wrangler (values via
# stdin so they never appear in the shell history / process list).
put_worker_secret() {
  local name="$1" value="$2"
...
banner "AI News Digest — send token setup"

# ── Stage 1: generate + set SEND_TOKEN ────────────────────────────────────
...
write_env SEND_TOKEN "$TOKEN"
put_worker_secret SEND_TOKEN "$TOKEN"
